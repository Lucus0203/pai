...
class Department extends CI_Controller {
	...
	public function index($dpid) {
                ...
                $this->load->database ();
                $sql="select student.*,department.name as department from ".$this->db->dbprefix('student')." student left join ".$this->db->dbprefix('department')." department on student.department_id=department.id where student.isdel = 2 and student.company_code='{$logininfo['company_code']}' ";
                $sql.=!empty($dpid)?" and (department_id=$dpid or department.parent_id = $dpid) ":'';
                //总人数
