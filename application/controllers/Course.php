...
class Course extends CI_Controller {
	...
	public function courselist() {
		...
		$parm['time_end'] = $this->input->get('time_end');
                ...
                if($parm['status']==4){//待发布
                    ...
                    $sql.=" and c.ispublic = 1 and (isapply_open!=1 or unix_timestamp(now()) < unix_timestamp(c.apply_start) ) ";
                }
                if(!empty($parm['keyword'])){
                    $sql.=" and (c.title like '%".$parm['keyword']."%' )";
                }
                if(!empty($parm['time_start'])){
                    $sql.=" and unix_timestamp(time_start) >= unix_timestamp('".$parm['time_start']."') ";
                }
                if(!empty($parm['time_end'])){
                    $sql.=" and unix_timestamp(time_start) <= unix_timestamp('".$parm['time_end']."') ";
                }
	...
	public function coursecreate(){
		...
		if(!empty($act)){
                    ...
                    if(empty($c['teacher_id'])){unset($c['teacher_id']);}
		...
		if(!empty($act)){
                    ...
                    $c=array('company_code'=>$logininfo['company_code'],
                        ...
                        'outline'=>$this->input->post('outline'),
                        'info'=>$this->input->post('info'));
                    if(empty($c['teacher_id'])){$c['teacher_id']=NULL;}
                    $config['upload_path'] = './uploads/course_img';
                    $config['allowed_types'] = 'gif|jpg|png|jpeg';
                    $config['file_name'] = $file_name = $logininfo['id'].date("YmdHis");
		...
		$act = $this->input->post('act');
		if(!empty($act)){
                    $oldcourse=$this->course_model->get_row(array('id'=>$id));
		...
		if(!empty($act)){
                    ...
                    if(empty($c['signout_start'])){$c['signout_start']=NULL;}
                    if(empty($c['signout_end'])){$c['signout_end']=NULL;}
                    $this->course_model->update($c,$id);
		}
                $course=$this->course_model->get_row(array('id'=>$id));
