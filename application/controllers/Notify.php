...
class Notify extends CI_Controller {
	...
	public function coursestart() {
        ...
        foreach ($courses as $c) {
            ...
            //微信初始化
            $companyToken=$this->companytokenwx_model->get_row(array('company_code'=>$c['company_code']));
            $this->load->library('wechat', $companyToken);
            //获取templateid
            $objTempid=$this->wechat->getTemplateId('TM00080');
...
如公司要求,请注意签到。",
                    ...
                    if($objTempid->errcode=='0') {
                        ...
                        $res = $this->wechat->templateSend($s['openid'], $templateid, $this->config->item('web_url') . 'course/courseinfo/' . $c['id'] . '.html', $wxdata);
                    }
                }
            }
        }
