...
class Student extends CI_Controller {
	...
	public function index($dpid) {
		...
		if(!empty($act)){
                    ...
                    if($this->student_model->get_count(array('mobile'=>$s['mobile']))>0){
                        $res['msg']='手机号已被使用';
                        $res['student']=$s;
		...
		$where="parent_id is null and company_code = '{$logininfo['company_code']}' ";
                ...
                $second_departments=$this->department_model->get_all(array('parent_id'=>$departments[0]['id']));
		...
		$act = $this->input->post('act');
                $res['student']=$this->student_model->get_row(array('id'=>$id));
		...
		if(!empty($act)){
                    ...
                    if($this->user_model->get_count("company_code = '{$s['company_code']}' and user_name = '{$s['user_name']}' and (student_id <> $id or student_id is null) ")>0){
                        $res['msg']='登录账号已被使用';
                        $res['student']=$s;
                    }else{
		...
		$where="parent_id is null and company_code = '{$logininfo['company_code']}' ";
                $departments=$this->department_model->get_all($where);
                $second_departments=$this->department_model->get_all(array('parent_id'=>$res['student']['department_parent_id']));
                foreach ($departments as $key => $d) {
